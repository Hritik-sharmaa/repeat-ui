...
program
  ...
  .option(
    "-v, --variant <variant>",
    "Component variant (js-css, js-tailwind, ts-css, ts-tailwind)",
    "ts-tailwind"
  )
  ...
  .action(async (componentName, options) => {
    try {
        ...
        const { component } = await inquirer.prompt([
          {
            ...
            choices: categoryComponents.map((key: string) => ({
      ...
      if (
        !componentInfo.variants[
          options.variant as keyof typeof componentInfo.variants
        ]
      ) {
        console.error(
          chalk.red(
            `Variant "${options.variant}" not available for this component.`
          )
        );
        console.log(
          chalk.yellow("Available variants:"),
          Object.keys(componentInfo.variants)
            .filter(
              (v) =>
                componentInfo.variants[v as keyof typeof componentInfo.variants]
            )
            .join(", ")
        );
        process.exit(1);
      ...
      const { files, dependencies } = await fetchComponentFiles(
        selectedComponent,
        componentInfo,
        options.variant
      );
      ...
      if (Object.keys(files).length === 0) {
        spinner.fail(
          chalk.red(
            "No component files found! Please check if the component exists."
          )
        );
        process.exit(1);
      }
      ...
      spinner.succeed(
        chalk.green(`Component "${selectedComponent}" installed successfully!`)
      );
      ...
      if (options.deps && dependencies.length > 0) {
        ...
        try {
          ...
          if (missingDeps.length > 0) {
            ...
            installSpinner.succeed(
              chalk.green(
                `Dependencies installed successfully! (${missingDeps.join(
                  ", "
                )})`
              )
            );
            ...
            installSpinner.succeed(
              chalk.green("All dependencies already installed!")
            );
          }
        } catch (error) {
          installSpinner.fail(chalk.red("Failed to install dependencies"));
          console.log(
            chalk.yellow("Please install manually:"),
        ...
        console.log(
          chalk.blue("💡 Tip: Use --deps flag to install automatically")
        );
      }
  ...
  .action((options) => {
    ...
    categories.forEach((category) => {
      if (!options.category || category === options.category) {
        ...
        const categoryComponents = getComponentsByCategory(category);
        categoryComponents.forEach((key: string) => {
          ...
          console.log(
            chalk.gray(
              `    Variants: ${Object.keys(component.variants)
                .filter(
                  (v) =>
                    component.variants[v as keyof typeof component.variants]
                )
                .join(", ")}`
            )
          );
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      console.log(
        chalk.blue("Configuration saved to:"),
        "./repeat-ui.config.json"
      );
      console.log(
        chalk.blue("Components will be installed to:"),
        "./components"
      );
      console.log(chalk.yellow("\nNext steps:"));
      console.log('  1. Run "repeat-ui list" to see available components');
      console.log(
        '  2. Run "repeat-ui add <component-name>" to install a component'
      );
    } catch (error) {
