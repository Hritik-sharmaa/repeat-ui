...
export function markComponentAsNew(subcategoryName, variantName, dateAdded) {
    const date = dateAdded || new Date().toISOString().split("T")[0];
    console.log(`Component ${subcategoryName}/${variantName} marked as new (${date})`);
    return {
    ...
    categories.forEach((category) => {
        category.subcategories.forEach((subcategory) => {
            subcategory.variants.forEach((variant) => {
                ...
                const daysOld = Math.floor((today.getTime() - addedDate.getTime()) / (1000 * 60 * 60 * 24));
                if (daysOld > 30) {
    ...
    switch (command) {
        case "list-new":
            ...
            newComponents.forEach((comp) => {
                console.log(`${comp.subcategory}/${comp.variant} - Added: ${comp.dateAdded}`);
            });
            break;
        case "list-expired":
            ...
            expiredComponents.forEach((comp) => {
                console.log(`${comp.subcategory}/${comp.variant} - Added: ${comp.dateAdded} (${comp.daysOld} days ago)`);
            });
            break;
        default:
            console.log("Usage:");
            console.log("  node component-dates.js list-new     - List all new components");
            console.log("  node component-dates.js list-expired - List components that are no longer new");
    }
}
