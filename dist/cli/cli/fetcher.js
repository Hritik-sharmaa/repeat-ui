...
export async function fetchComponentFiles(componentKey, component, variant) {
    const files = {};
    ...
    try {
        ...
        const componentLocalPath = path.join(process.cwd(), "src/app/components/content", component.category, variant, componentFolderName, `${component.files.component}.${componentExt}`);
        ...
        if (componentContent) {
            ...
            try {
                const { dependencies: analyzedDeps } = await analyzeComponentDependencies(componentContent);
        ...
        if (style === "css" && component.files.styles) {
            ...
            const stylesLocalPath = path.join(process.cwd(), "src/app/components/content", component.category, variant, componentFolderName, component.files.styles);
        ...
        const demoLocalPath = path.join(process.cwd(), "src/app/components/content", component.category, variant, componentFolderName, `${component.files.demo}.${demoExt}`);
...
function isBuiltInPackage(packageName) {
    ...
    return builtInPackages.some((builtIn) => packageName === builtIn || packageName.startsWith(`${builtIn}/`));
}
